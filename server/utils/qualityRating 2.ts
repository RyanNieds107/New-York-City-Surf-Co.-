...
function getWindQualityForLido(windSpeedKt: number, windDirectionDeg: number): number {
  ...
  if (isSideOffshore(normalized)) {
    let score: number;
    if (windSpeedKt <= 12) {
      score = 5; // Light WNW still somewhat favorable
    } else if (windSpeedKt <= 18) {
      score = 0; // Neutral - moderate WNW
    } else {
      score = -15; // Strong WNW creates choppy, messy conditions
    }
...
export function scoreWind(
  ...
  if (isOffshore) {
    ...
    // All spots get penalties for >18kts WNW (choppy, messy conditions)
    if (isRockaway) {
      ...
      if (windSpeedKt <= 12) {
        ...
        return -15; // Strong WNW creates choppy conditions even at Rockaway
      }
    } else {
      // Long Beach: More conservative - anything >12kts is neutral, >18kts gets penalty
      if (windSpeedKt <= 12) {
        return 5;
      } else if (windSpeedKt <= 18) {
        return 0; // Neutral for moderate WNW
      } else {
        return -15; // Strong WNW creates choppy, messy conditions
      }
    }
  } else if (isWestCross) {
